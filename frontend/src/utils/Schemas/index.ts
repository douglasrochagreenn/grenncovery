...
const passwordConfirmedSchema = z
  .string()
  .min(6, { message: "A senha deve ter no mínimo 6 caracteres" });
...
const complementoSchema = z
  .string()
  .min(1, "Complemento é obrigatório")
  .optional();
...
const manufacturingYearSchema = z
  .string()
...
const mileageSchema = z
  .string()
  .min(1, "Quilometragem do veículo é obrigatório");
const vechicleDescriptionSchema = z
  .string()
  .min(1, "Descrição do veículo é obrigatório");
const VechileValueSchema = z.string().min(1, "Valor do veículo é obrigatório");
const vechicleInformationSchema = z
  .string()
  .min(1, "Demais informações do veículo é obrigatório");
// --------------------------------->>> STEP 3
const optionalSchema = z
  .array(z.string())
  .nonempty("Os opcionais não podem estar vazio");
...
const vechicleTypeSchema = z
  .string()
  .min(1, "Tipo do veículo é obrigatorio")
  .default("Carro");
...
export type ISendLogin = z.infer<typeof rawSchemaLogin>;
